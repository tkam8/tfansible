...
parser.add_argument("-f", "--filename", default="/home/tfansible/repos.json", help="The JSON repo file")
...
log = safe_open('/home/tfansible/log/cloneGitRepos.log', 'a')
...
for i in range(0, numrepos):
	...
	if 'localdir' not in obj:
		obj["localdir"] = '/home/tfansible/%s' % obj["name"]
	...
	install_fn = '%s/tfansible_install.sh' % obj["localdir"]
	...
	if os.path.exists(install_fn):
		...
		exitcode = call(['/bin/bash','-c','%s/tfansible_install.sh' % obj["localdir"]], shell=False, stdout=install_log, stderr=install_log, cwd=obj["localdir"])
		install_log.close()
		if exitcode > 0:
...
done = safe_open('/tfansboot/TFANSIBLE_ENV', 'a')
done.write('TFANSIBLE_CLONE_DONE=1\n')
done.close()