...
def process_repo_file(filename, stack={"repos":[]}, depth=0):
	...
	if 'include' in repo_dict:
		for includerepo in repo_dict["include"]:
			...
			stack = process_repo_file("/tmp/tfansible-repo/images/%s/fs/etc/tfansiblerepo.d/%s.json" % (includerepo, includerepo), stack, depth+1)
...
parser = argparse.ArgumentParser(description='Script to update tfansible container repo list')
...
with open('/home/tfansible/repos.json', 'w') as out:
